...
A app procura nestas localizações:
...
Podes gerar um `.wvd` dentro da app em **Estado → Gerar .wvd**, usando:
...
### Como obter os ficheiros para gerar `.wvd`

Há duas formas comuns.

#### Opção A — Extrair o teu próprio CDM L3

Esta é a opção mais correta quando queres usar material teu.

1. Usa um dispositivo Android ou emulador Android.
2. Extrai o CDM L3 do teu ambiente.
3. Obtém estes ficheiros:
   - `private_key.pem`
   - `client_id.bin`
4. Abre a app.
5. Vai a **Estado → Gerar .wvd**.
6. Seleciona os dois ficheiros.
7. Clica em **Gerar .wvd**.
...
#### Opção B — Usar um CDM já existente

Também podes usar um CDM que já tenhas guardado localmente ou obtido de uma fonte em que confies.

1. Garante que tens `private_key.pem` e `client_id.bin`.
2. Usa a opção **Gerar .wvd** dentro da app.
3. Guarda o `.wvd` em `secrets/`.

Referência pública frequentemente usada:

```text
https://forum.videohelp.com/threads/413719-Ready-to-use-CDMs-available-here%21
```

> A utilização de CDMs de terceiros é da tua inteira responsabilidade.
